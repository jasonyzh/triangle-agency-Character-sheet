...
app.use(require('./routes/npc-templates'));
app.use(require('./routes/image-library'));
app.use(require('./routes/boards'));
app.use(require('./routes/destruction-tracks'));

app.get('/', (req, res) => res.redirect('/desktop.html'));   // 默认进桌面终端（未登录时 desktop 自带登录层）
...
io.on('connection', socket => {
    ...
    socket.on('weather:update', data => {
        const mId = (data && data.missionId) || socket.currentMissionId;
        if (mId) {
            socket.to('mission-' + mId + '-ply').emit('weather:update', data);
        }
    });

    socket.on('dice:roll', data => {
        if (data.missionId) {
            socket.to('mission-' + data.missionId + '-mgr').emit('dice:roll', data);
            socket.to('mission-' + data.missionId + '-ply').emit('dice:roll', data);
        }
    });

    socket.on('mail:notify', data => {
        if (data.userId) {
            socket.to('user-' + data.userId).emit('mail:new', {});
        }
    });
