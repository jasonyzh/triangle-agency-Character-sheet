...
router.post('/api/image-library/upload', authenticateToken, requireRole(ROLE.MANAGER), upload.single('image'), (req, res) => {
    ...
    db.run('INSERT INTO image_library (id, filename, original_name, category, folder, uploaded_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [id, relPath, req.file.originalname, category, folder, req.user.userId, Date.now()],
        function(err) {
...
router.delete('/api/image-library/:id', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    db.get('SELECT filename FROM image_library WHERE id = ?', [req.params.id], (err, row) => {
        if (!row) return res.status(404).json({ success: false, message: '图片不存在' });
        const filePath = path.join(UPLOADS_DIR, row.filename);
        if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
        db.run('DELETE FROM image_library WHERE id = ?', [req.params.id], function(err) {
            if (err) return res.status(500).json({ success: false, message: err.message });
...
router.put('/api/image-library/:id', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    ...
    db.run('UPDATE image_library SET original_name = ? WHERE id = ?', [name, req.params.id], function(err) {
        if (err) return res.status(500).json({ success: false, message: err.message });
        res.json({ success: true });
    });
});
...
// 文件夹列表（只读磁盘根目录下包含 NPC+MAP 子目录的文件夹）
...
router.put('/api/image-library/folders', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    ...
    try {
        ...
        db.run('UPDATE image_library SET folder = ? WHERE folder = ?', [safeNew, oldName]);
