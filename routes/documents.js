...
router.put('/api/manager/character/:charId/permissions', authenticateToken, requireRole(ROLE.MANAGER), async (req, res) => {
    try {
        ...
        if (newFiles.length > 0) {
            // 通过 socket 实时通知玩家邮箱更新
            const charRow = db.prepare('SELECT user_id FROM characters WHERE id = ?').get(charId);
            if (charRow && global.io) {
                global.io.to('user-' + charRow.user_id).emit('mail:new', {});
            }
        }
        // 对所有已勾选文件尝试把对应模板能力写入角色（幂等：同名跳过、模板缺失跳过）。
        // 覆盖"先授权后补模板"的场景——补录模板后再次保存权限即可补齐能力。
        permissions.forEach(file => {
