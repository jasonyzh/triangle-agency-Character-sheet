...
router.post('/api/manager/send-mail', authenticateToken, requireRole(ROLE.MANAGER), async (req, res) => {
    ...
    for (var i = 0; i < characterIds.length; i++) {
        ...
        await new Promise(function(resolve) {
            db.get('SELECT user_id, data FROM characters WHERE id = ?', [charId], function(err, row) {
                ...
                db.run('INSERT INTO character_messages (character_id, sender_id, sender_name, subject, content, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    [charId, req.user.userId, senderName, subject, content, 'mail', now],
                    function(err) { if (!err) { sentCount++; recipientIds.push(row.user_id); } resolve(); }
                );
            });
        });
    }
