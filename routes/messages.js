...
router.get('/api/character/:id/highwall-files', authenticateToken, (req, res) => {
    ...
    try {
        ...
        let hasAccess = req.user.role >= ROLE.SUPER_ADMIN || req.user.userId === row.user_id;
        if (!hasAccess && req.user.role >= ROLE.MANAGER) {
            hasAccess = !!db.prepare('SELECT 1 FROM character_authorizations WHERE character_id = ? AND manager_id = ?').get(charId, req.user.userId);
        }

        if (!hasAccess) return res.status(403).json([]);

        const rows = db.prepare('SELECT filename, granted_at FROM character_document_permissions WHERE character_id = ?').all(charId);
...
router.get('/api/character/:id/check-a1', authenticateToken, (req, res) => {
    ...
    try {
        ...
        const perm = db.prepare(`SELECT 1 FROM character_document_permissions
                WHERE character_id = ? AND LOWER(filename) = 'a1.md'`).get(charId);
...
router.get('/api/character/:id/messages', authenticateToken, (req, res) => {
    ...
    try {
        ...
        const rows = db.prepare(`SELECT * FROM character_messages WHERE character_id = ? AND (message_type IS NULL OR message_type != 'sent') ORDER BY created_at DESC`).all(charId);
...
router.get('/api/character/:id/sent-messages', authenticateToken, (req, res) => {
    const charId = req.params.id;
...
router.post('/api/manager/character/:charId/message', authenticateToken, requireRole(ROLE.MANAGER), async (req, res) => {
    try {
        ...
        const row = db.prepare('SELECT name, username FROM users WHERE id = ?').get(managerId);
        const sender = row ? (row.name || row.username) : '未知';

        const result = db.prepare('INSERT INTO character_messages (character_id, sender_id, sender_name, subject, content, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(charId, managerId, sender, subject, content, Date.now());
...
router.put('/api/character/:charId/message/:msgId/read', authenticateToken, (req, res) => {
    ...
    try {
        ...
        db.prepare('UPDATE character_messages SET read = 1 WHERE id = ? AND character_id = ?').run(msgId, charId);
...
router.delete('/api/character/:id/message/:msgId', authenticateToken, (req, res) => {
    ...
    try {
        const row = db.prepare('SELECT * FROM character_messages WHERE id = ? AND character_id = ?').get(msgId, charId);
        if (!row) return res.status(404).json({ success: false, message: '消息不存在' });
        db.prepare('DELETE FROM character_messages WHERE id = ? AND character_id = ?').run(msgId, charId);
        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
...
router.post('/api/character/:id/u2-unleash', authenticateToken, (req, res) => {
    ...
    try {
        ...
        const perm = db.prepare(`SELECT 1 FROM character_document_permissions
                WHERE character_id = ? AND LOWER(filename) = 'u2.md'`).get(charId);
