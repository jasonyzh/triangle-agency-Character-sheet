...
// 把模板能力写入角色数据（幂等：同名跳过）
function pushAnomToCharData(data, tmpl) {
    if (!data.anoms) data.anoms = [];
    if (data.anoms.some(a => a.name === tmpl.name)) return false;
    let p1 = [], p2 = [], p3 = [], alist = [];
    try { p1 = JSON.parse(tmpl.p1); } catch (e) {}
    try { p2 = JSON.parse(tmpl.p2); } catch (e) {}
    try { p3 = JSON.parse(tmpl.p3); } catch (e) {}
    try { alist = JSON.parse(tmpl.alist); } catch (e) {}
    data.anoms.push({
        name: tmpl.name, trig: tmpl.trig, qual: tmpl.qual,
        succ: tmpl.succ, fail: tmpl.fail, passive: tmpl.passive ? true : false, chk: tmpl.chk ? true : false,
        subOn: tmpl.sub_on ? true : false, sub: tmpl.sub || '',
        listOn: tmpl.list_on ? true : false, listName: tmpl.alist_name || '', list: alist,
        tdesc: tmpl.tdesc, t1: tmpl.t1, t1v: tmpl.t1v,
        t2: tmpl.t2, t2v: tmpl.t2v, t3: tmpl.t3, t3v: tmpl.t3v, p1: p1, p2: p2, p3: p3
    });
    return true;
}

// 模板创建/更新后：把能力补写给所有已授权对应高墙文件（doc_filename）的角色，
// 解决"先授权高墙文件、后补模板"时能力无法出现在角色卡中的问题
function syncAnomalyToAuthorizedChars(tmpl) {
    if (!tmpl || !tmpl.doc_filename) return;
    try {
        const rows = db.prepare(`
            SELECT c.id, c.data FROM characters c
            JOIN character_document_permissions cdp ON cdp.character_id = c.id
            WHERE cdp.filename = ? AND c.branch_id = ?
        `).all(tmpl.doc_filename, tmpl.branch_id);
        rows.forEach(row => {
            try {
                const data = JSON.parse(row.data);
                if (pushAnomToCharData(data, tmpl)) {
...
router.post('/api/anomaly-templates', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    try {
        const { branchId, name, trig, qual, succ, fail, passive, chk, tdesc, t1, t1v, t2, t2v, t3, t3v, p1, p2, p3, subOn, sub, listOn, listName, list, docFilename } = req.body;
        ...
        db.prepare(`INSERT INTO anomaly_templates (id, branch_id, name, trig, qual, succ, fail, passive, chk, tdesc, t1, t1v, t2, t2v, t3, t3v, p1, p2, p3, sub_on, sub, list_on, alist_name, alist, doc_filename, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
            id, branchId, name, trig || '', qual || '', succ || '', fail || '', passive ? 1 : 0, chk ? 1 : 0, tdesc || '', t1 || '', t1v || '', t2 || '', t2v || '', t3 || '', t3v || '', JSON.stringify(p1 || []), JSON.stringify(p2 || []), JSON.stringify(p3 || []), subOn ? 1 : 0, sub || '', listOn ? 1 : 0, listName || '', JSON.stringify(list || []), docFilename || '', now
        );
        // 补写能力给所有已授权对应高墙文件的角色
        syncAnomalyToAuthorizedChars(db.prepare('SELECT * FROM anomaly_templates WHERE id = ?').get(id));
...
router.put('/api/anomaly-templates/:id', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    try {
        const { name, trig, qual, succ, fail, passive, chk, tdesc, t1, t1v, t2, t2v, t3, t3v, p1, p2, p3, subOn, sub, listOn, listName, list, docFilename } = req.body;
        ...
        const result = db.prepare(`UPDATE anomaly_templates SET name=?, trig=?, qual=?, succ=?, fail=?, passive=?, chk=?, tdesc=?, t1=?, t1v=?, t2=?, t2v=?, t3=?, t3v=?, p1=?, p2=?, p3=?, sub_on=?, sub=?, list_on=?, alist_name=?, alist=?, doc_filename=? WHERE id=?`).run(
            name || '', trig || '', qual || '', succ || '', fail || '', passive ? 1 : 0, chk ? 1 : 0, tdesc || '', t1 || '', t1v || '', t2 || '', t2v || '', t3 || '', t3v || '', JSON.stringify(p1 || []), JSON.stringify(p2 || []), JSON.stringify(p3 || []), subOn ? 1 : 0, sub || '', listOn ? 1 : 0, listName || '', JSON.stringify(list || []), docFilename || '', req.params.id
        );
        if (result.changes === 0) return res.status(404).json({ success: false, message: '模板不存在' });
        // 更新后同步能力给所有已授权对应高墙文件的角色
        syncAnomalyToAuthorizedChars(db.prepare('SELECT * FROM anomaly_templates WHERE id = ?').get(req.params.id));
    ...
    try {
        ...
        try {
            ...
            data.anoms.push({
                ...
                succ: tmpl.succ,
                fail: tmpl.fail,
                passive: tmpl.passive ? true : false,
                chk: tmpl.chk ? true : false,
                subOn: tmpl.sub_on ? true : false,
                sub: tmpl.sub || '',
                listOn: tmpl.list_on ? true : false,
                listName: tmpl.alist_name || '',
                list: alist,
                tdesc: tmpl.tdesc,
                t1: tmpl.t1,
                t1v: tmpl.t1v,
