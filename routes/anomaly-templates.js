...
router.post('/api/anomaly-templates', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    try {
        const { branchId, name, trig, qual, succ, fail, passive, chk, tdesc, t1, t1v, t2, t2v, t3, t3v, p1, p2, p3, docFilename } = req.body;
        ...
        db.prepare(`INSERT INTO anomaly_templates (id, branch_id, name, trig, qual, succ, fail, passive, chk, tdesc, t1, t1v, t2, t2v, t3, t3v, p1, p2, p3, doc_filename, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
            id, branchId, name, trig || '', qual || '', succ || '', fail || '', passive ? 1 : 0, chk ? 1 : 0, tdesc || '', t1 || '', t1v || '', t2 || '', t2v || '', t3 || '', t3v || '', JSON.stringify(p1 || []), JSON.stringify(p2 || []), JSON.stringify(p3 || []), docFilename || '', now
        );
        // 补写能力给所有已授权对应高墙文件的角色
...
router.put('/api/anomaly-templates/:id', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    try {
        const { name, trig, qual, succ, fail, passive, chk, tdesc, t1, t1v, t2, t2v, t3, t3v, p1, p2, p3, docFilename } = req.body;
        ...
        const result = db.prepare(`UPDATE anomaly_templates SET name=?, trig=?, qual=?, succ=?, fail=?, passive=?, chk=?, tdesc=?, t1=?, t1v=?, t2=?, t2v=?, t3=?, t3v=?, p1=?, p2=?, p3=?, doc_filename=? WHERE id=?`).run(
            name || '', trig || '', qual || '', succ || '', fail || '', passive ? 1 : 0, chk ? 1 : 0, tdesc || '', t1 || '', t1v || '', t2 || '', t2v || '', t3 || '', t3v || '', JSON.stringify(p1 || []), JSON.stringify(p2 || []), JSON.stringify(p3 || []), docFilename || '', req.params.id
        );
