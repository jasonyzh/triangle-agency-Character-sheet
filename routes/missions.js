...
router.get('/api/manager/mission/:id/characters-full', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    ...
    db.get('SELECT * FROM field_missions WHERE id = ?', [missionId], (err, mission) => {
        ...
        const authCheck = req.user.role >= ROLE.SUPER_ADMIN
            ? Promise.resolve(true)
            : new Promise(resolve => {
                db.get('SELECT 1 FROM user_branches WHERE user_id = ? AND branch_id = ?', [req.user.userId, mission.branch_id], (e, r) => resolve(!!r));
            });

        authCheck.then(authorized => {
