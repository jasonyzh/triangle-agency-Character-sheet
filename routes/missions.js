const express = require('express');
const router = express.Router();
const { db } = require('../db/init');
const { ROLE } = require('../constants');
const { authenticateToken, requireRole } = require('../middleware/auth');

router.post('/api/manager/mission', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const { name, description, missionType, characterIds, branchId } = req.body;

    if (!name || !name.trim()) {
        return res.status(400).json({ success: false, message: '任务名称不能为空' });
    }

    const validType = ['containment', 'sweep'].includes(missionType) ? missionType : 'containment';

    const missionId = Date.now().toString();
    const now = Date.now();

    try {
        const insertMission = db.prepare(`INSERT INTO field_missions
            (id, name, description, mission_type, status, created_by, created_at, updated_at, report_status, branch_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        const insertMember = db.prepare('INSERT OR IGNORE INTO field_mission_members (mission_id, character_id, member_status, joined_at) VALUES (?, ?, ?, ?)');

        const doInsert = db.transaction(() => {
            insertMission.run(missionId, name.trim(), description || '', validType, 'active', req.user.userId, now, now, 'none', branchId || null);
            if (characterIds && characterIds.length > 0) {
                characterIds.forEach(charId => {
                    insertMember.run(missionId, charId, '待命', now);
                });
            }
        });
        doInsert();

        res.json({ success: true, missionId });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.get('/api/manager/missions', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const managerId = req.user.userId;
    const statusFilter = req.query.status;
    const branchId = req.query.branchId;

    let sql = 'SELECT * FROM field_missions';
    let params = [];
    const conditions = [];

    if (branchId) {
        conditions.push('branch_id = ?');
        params.push(branchId);
    } else if (req.user.role < ROLE.SUPER_ADMIN) {
        conditions.push('created_by = ?');
        params.push(managerId);
    }
    
    if (statusFilter === 'active' || statusFilter === 'archived') {
        conditions.push('status = ?');
        params.push(statusFilter);
    }
    
    if (conditions.length > 0) {
        sql += ' WHERE ' + conditions.join(' AND ');
    }

    sql += ' ORDER BY created_at DESC';

    try {
        const missions = db.prepare(sql).all(...params);

        if (!missions || missions.length === 0) {
            return res.json([]);
        }

        const membersStmt = db.prepare(`
            SELECT fmm.character_id, c.data, c.user_id, fmm.member_status, fmm.joined_at
            FROM field_mission_members fmm
            JOIN characters c ON fmm.character_id = c.id
            WHERE fmm.mission_id = ?
        `);

        const result = missions.map(mission => {
            const members = membersStmt.all(mission.id);
            const memberList = (members || []).map(m => {
                let d = {};
                try { d = JSON.parse(m.data); } catch(e) {}
                return {
                    character_id: m.character_id,
                    user_id: m.user_id,
                    name: d.pName || '未命名',
                    member_status: m.member_status,
                    joined_at: m.joined_at
                };
            });
            
            return {
                ...mission,
                members: memberList,
                mission_type: mission.mission_type || 'containment'
            };
        });

        result.sort((a, b) => b.created_at - a.created_at);
        res.json(result);
    } catch (err) {
        console.error(err);
        return res.status(500).json([]);
    }
});

router.put('/api/manager/mission/:id', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const { name, description, status, missionType, chaosValue, scatterValue, weather } = req.body;

    try {
        const mission = db.prepare('SELECT * FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (req.user.role < ROLE.SUPER_ADMIN) {
            const row = db.prepare('SELECT branch_id FROM user_branches WHERE user_id = ? AND branch_id = ?').get(req.user.userId, mission.branch_id);
            if (!row) return res.status(403).json({ success: false, message: '无权修改此任务' });
        }

        const updates = [];
        const params = [];

        if (name !== undefined) {
            updates.push('name = ?');
            params.push(name.trim());
        }
        if (description !== undefined) {
            updates.push('description = ?');
            params.push(description);
        }
        if (status !== undefined && ['active', 'archived'].includes(status)) {
            updates.push('status = ?');
            params.push(status);
        }
        if (missionType !== undefined && ['containment', 'sweep'].includes(missionType)) {
            updates.push('mission_type = ?');
            params.push(missionType);
        }
        if (chaosValue !== undefined && !isNaN(parseInt(chaosValue))) {
            updates.push('chaos_value = ?');
            params.push(parseInt(chaosValue));
        }
        if (scatterValue !== undefined && !isNaN(parseInt(scatterValue))) {
            updates.push('scatter_value = ?');
            params.push(parseInt(scatterValue));
        }
        if (weather !== undefined) {
            updates.push('weather = ?');
            params.push(weather);
        }

        if (updates.length === 0) {
            return res.json({ success: true });
        }

        updates.push('updated_at = ?');
        params.push(Date.now());
        params.push(missionId);

        db.prepare(`UPDATE field_missions SET ${updates.join(', ')} WHERE id = ?`).run(...params);
        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.delete('/api/manager/mission/:id', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权删除此任务' });
        }

        db.prepare('DELETE FROM field_missions WHERE id = ?').run(missionId);
        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.post('/api/manager/mission/:id/member', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const { characterId } = req.body;

    if (!characterId) {
        return res.status(400).json({ success: false, message: '角色卡ID不能为空' });
    }

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权修改此任务' });
        }

        const existing = db.prepare('SELECT 1 FROM field_mission_members WHERE mission_id = ? AND character_id = ?').get(missionId, characterId);
        if (existing) {
            return res.status(400).json({
                success: false,
                message: '该特工已在本任务中'
            });
        }

        const now = Date.now();
        const doInsert = db.transaction(() => {
            db.prepare('INSERT INTO field_mission_members (mission_id, character_id, member_status, joined_at) VALUES (?, ?, ?, ?)').run(missionId, characterId, '待命', now);
            db.prepare('UPDATE field_missions SET updated_at = ? WHERE id = ?').run(now, missionId);
        });
        doInsert();

        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.delete('/api/manager/mission/:id/member/:charId', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const charId = req.params.charId;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权修改此任务' });
        }

        const now = Date.now();
        const doDelete = db.transaction(() => {
            db.prepare('DELETE FROM field_mission_members WHERE mission_id = ? AND character_id = ?').run(missionId, charId);
            db.prepare('UPDATE field_missions SET updated_at = ? WHERE id = ?').run(now, missionId);
        });
        doDelete();

        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.get('/api/manager/mission/:id/inbox', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权访问此任务' });
        }

        const messages = db.prepare(`
            SELECT * FROM mission_inbox
            WHERE mission_id = ?
            ORDER BY created_at DESC
        `).all(missionId);

        res.json({ success: true, messages: messages || [] });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.get('/api/manager/mission/:id/inbox/unread-count', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权访问此任务' });
        }

        const row = db.prepare('SELECT COUNT(*) as count FROM mission_inbox WHERE mission_id = ? AND read = 0').get(missionId);
        res.json({ success: true, count: row ? row.count : 0 });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.put('/api/manager/mission/:id/inbox/:msgId/read', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const msgId = req.params.msgId;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权操作' });
        }

        db.prepare('UPDATE mission_inbox SET read = 1 WHERE id = ? AND mission_id = ?').run(msgId, missionId);
        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.delete('/api/manager/mission/:id/inbox/:msgId', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const msgId = req.params.msgId;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权操作' });
        }

        db.prepare('DELETE FROM mission_inbox WHERE id = ? AND mission_id = ?').run(msgId, missionId);
        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.get('/api/manager/mission/:id/reports', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权访问此任务' });
        }

        const reports = db.prepare(`
            SELECT mr.*, c.data as char_data
            FROM mission_reports mr
            LEFT JOIN characters c ON mr.submitted_by = c.id
            WHERE mr.mission_id = ?
            ORDER BY mr.submitted_at DESC
        `).all(missionId);

        const result = (reports || []).map(r => {
            let charName = '未知特工';
            try {
                const charData = JSON.parse(r.char_data || '{}');
                charName = charData.pName || '未命名';
            } catch(e) {}

            return {
                id: r.id,
                missionId: r.mission_id,
                submittedBy: r.submitted_by,
                submitterName: charName,
                originalData: r.original_data ? JSON.parse(r.original_data) : null,
                revisedData: r.revised_data ? JSON.parse(r.revised_data) : null,
                annotations: r.annotations ? JSON.parse(r.annotations) : [],
                rating: r.rating,
                scatterValue: r.scatter_value,
                status: r.status,
                submittedAt: r.submitted_at,
                reviewedAt: r.reviewed_at,
                sentAt: r.sent_at
            };
        });

        res.json({ success: true, reports: result });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.put('/api/manager/mission/:id/report/:reportId', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const reportId = req.params.reportId;
    const { revisedData, annotations, rating, scatterValue } = req.body;

    try {
        const mission = db.prepare('SELECT created_by FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权操作' });
        }

        const updates = ['reviewed_at = ?', 'status = ?'];
        const params = [Date.now(), 'reviewed'];

        if (revisedData !== undefined) {
            updates.push('revised_data = ?');
            params.push(JSON.stringify(revisedData));
        }
        if (annotations !== undefined) {
            updates.push('annotations = ?');
            params.push(JSON.stringify(annotations));
        }
        if (rating !== undefined) {
            updates.push('rating = ?');
            params.push(rating);
        }
        if (scatterValue !== undefined) {
            updates.push('scatter_value = ?');
            params.push(parseInt(scatterValue) || 0);
        }

        params.push(reportId, missionId);

        const now = Date.now();
        const doUpdate = db.transaction(() => {
            db.prepare(`UPDATE mission_reports SET ${updates.join(', ')} WHERE id = ? AND mission_id = ?`).run(...params);
            db.prepare('UPDATE field_missions SET report_status = ?, updated_at = ? WHERE id = ?').run('reviewed', now, missionId);
        });
        doUpdate();

        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.post('/api/manager/mission/:id/report/:reportId/send', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;
    const reportId = req.params.reportId;

    try {
        const mission = db.prepare('SELECT created_by, name FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权操作' });
        }

        const report = db.prepare('SELECT * FROM mission_reports WHERE id = ? AND mission_id = ?').get(reportId, missionId);
        if (!report) return res.status(404).json({ success: false, message: '报告不存在' });

        const now = Date.now();

        const doSend = db.transaction(() => {
            db.prepare('UPDATE mission_reports SET status = ?, sent_at = ? WHERE id = ?').run('sent', now, reportId);
            db.prepare('UPDATE field_missions SET report_status = ?, updated_at = ? WHERE id = ?').run('sent', now, missionId);

            if (report.submitted_by) {
                const subject = `[评级通知] ${mission.name} - 任务报告已评审`;
                const content = `您提交的任务报告已被经理评审。\n\n任务: ${mission.name}\n评级: ${report.rating || '未评级'}\n逸散端: ${report.scatter_value || 0}`;
                db.prepare('INSERT INTO character_messages (character_id, sender_id, sender_name, subject, content, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(report.submitted_by, req.user.userId, '任务系统', subject, content, 'system', now);
            }
        });
        doSend();

        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.post('/api/manager/mission/:id/archive', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;

    try {
        const mission = db.prepare('SELECT * FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        if (mission.created_by !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ success: false, message: '无权操作' });
        }

        const reportCount = db.prepare('SELECT COUNT(*) as count FROM mission_reports WHERE mission_id = ?').get(missionId);
        if (reportCount && reportCount.count > 0) {
            const unsent = db.prepare('SELECT COUNT(*) as unsent FROM mission_reports WHERE mission_id = ? AND status != ?').get(missionId, 'sent');
            if (unsent && unsent.unsent > 0) {
                return res.status(400).json({
                    success: false,
                    message: '存在未发送的报告评级，请先完成所有报告评审并发送给特工'
                });
            }
        }

        const now = Date.now();
        db.prepare('UPDATE field_missions SET status = ?, updated_at = ? WHERE id = ?').run('archived', now, missionId);
        res.json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.get('/api/character/:charId/mission', authenticateToken, (req, res) => {
    const charId = req.params.charId;

    try {
        const char = db.prepare('SELECT user_id FROM characters WHERE id = ?').get(charId);
        if (!char) return res.status(404).json({ error: '角色不存在' });

        if (char.user_id !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ error: '无权访问' });
        }

        const mission = db.prepare(`
            SELECT fm.*, fmm.member_status
            FROM field_mission_members fmm
            JOIN field_missions fm ON fmm.mission_id = fm.id
            WHERE fmm.character_id = ? AND fm.status = 'active'
            ORDER BY fm.created_at DESC
            LIMIT 1
        `).get(charId);
        if (!mission) {
            return res.json({ inMission: false });
        }

        const members = db.prepare(`
            SELECT fmm.character_id, fmm.member_status, c.data
            FROM field_mission_members fmm
            JOIN characters c ON fmm.character_id = c.id
            WHERE fmm.mission_id = ?
        `).all(mission.id);

        const teammates = (members || []).map(m => {
            let d = {};
            try { d = JSON.parse(m.data); } catch(e) {}
            return {
                characterId: m.character_id,
                characterName: d.pName || '未命名',
                status: m.member_status,
                isMe: m.character_id === charId
            };
        });

        res.json({
            inMission: true,
            missionId: mission.id,
            missionName: mission.name,
            myStatus: mission.member_status,
            teammates
        });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

/* 玩家任务历史：参与过的全部任务（含归档）+ 本人提交的报告/收容物 */
router.get('/api/character/:charId/mission-history', authenticateToken, (req, res) => {
    const charId = req.params.charId;

    try {
        const char = db.prepare('SELECT user_id FROM characters WHERE id = ?').get(charId);
        if (!char) return res.status(404).json({ error: '角色不存在' });

        if (char.user_id !== req.user.userId && req.user.role < ROLE.SUPER_ADMIN) {
            return res.status(403).json({ error: '无权访问' });
        }

        const missions = db.prepare(`
            SELECT fm.id, fm.name, fm.description, fm.status, fm.mission_type, fm.chaos_value, fm.scatter_value,
                   fm.report_status, fmm.member_status, fmm.joined_at
            FROM field_mission_members fmm
            JOIN field_missions fm ON fmm.mission_id = fm.id
            WHERE fmm.character_id = ?
            ORDER BY fmm.joined_at DESC
        `).all(charId);

        const reports = {};
        db.prepare('SELECT * FROM mission_reports WHERE submitted_by = ? ORDER BY submitted_at DESC').all(charId)
            .forEach(r => { if (!reports[r.mission_id]) reports[r.mission_id] = r; });
        const containments = {};
        db.prepare(`SELECT * FROM mission_inbox WHERE sender_character_id = ? AND message_type = 'containment' ORDER BY created_at DESC`).all(charId)
            .forEach(c => { if (!containments[c.mission_id]) containments[c.mission_id] = c; });

        const list = (missions || []).map(m => {
            const r = reports[m.id];
            const c = containments[m.id];
            let reportData = null;
            if (r && r.original_data) {
                try { reportData = JSON.parse(r.original_data); } catch (e) {}
            }
            let annotations = [];
            if (r && r.annotations) {
                try { annotations = JSON.parse(r.annotations); } catch (e) {}
            }
            return {
                id: m.id,
                name: m.name,
                description: m.description,
                status: m.status,
                missionType: m.mission_type || 'containment',
                reportStatus: m.report_status || 'none',
                chaosValue: m.chaos_value || 0,
                scatterValue: m.scatter_value || 0,
                joinedAt: m.joined_at,
                myReport: r ? {
                    rating: r.rating,
                    scatterValue: r.scatter_value,
                    annotations: annotations,
                    status: r.status,
                    reportData: reportData,
                    submittedAt: r.submitted_at
                } : null,
                myContainment: c ? {
                    name: (c.subject || '').replace(/^\[收容物\]\s*/, ''),
                    content: c.content,
                    createdAt: c.created_at
                } : null
            };
        });

        res.json(list);
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

router.get('/api/manager/mission/:id/characters-full', authenticateToken, requireRole(ROLE.MANAGER), (req, res) => {
    const missionId = req.params.id;

    try {
        const mission = db.prepare('SELECT * FROM field_missions WHERE id = ?').get(missionId);
        if (!mission) return res.status(404).json({ success: false, message: '任务不存在' });

        const authorized = req.user.role >= ROLE.SUPER_ADMIN
            || !!db.prepare('SELECT 1 FROM user_branches WHERE user_id = ? AND branch_id = ?').get(req.user.userId, mission.branch_id);

        if (!authorized) return res.status(403).json({ success: false, message: '无权访问此任务' });

        const members = db.prepare(`
            SELECT fmm.character_id, fmm.member_status, c.data, u.name as owner_name
            FROM field_mission_members fmm
            JOIN characters c ON fmm.character_id = c.id
            LEFT JOIN users u ON c.user_id = u.id
            WHERE fmm.mission_id = ?
        `).all(missionId);

        const characters = (members || []).map(m => {
            let d = {};
            try { d = JSON.parse(m.data); } catch(e) {}
            return {
                characterId: m.character_id,
                memberStatus: m.member_status,
                data: d,
                ownerName: m.owner_name || '未知'
            };
        });

        res.json({
            success: true,
            mission: {
                id: mission.id,
                name: mission.name,
                description: mission.description,
                status: mission.status,
                chaos_value: mission.chaos_value || 0,
                scatter_value: mission.scatter_value || 0,
                weather: mission.weather || ''
            },
            characters
        });
    } catch (err) {
        return res.status(500).json({ success: false, message: '数据库错误' });
    }
});

module.exports = router;
