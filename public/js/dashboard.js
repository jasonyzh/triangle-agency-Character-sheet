...
async function loadBranchesForApply() {
    try {
        ...
        container.innerHTML = data.branches.map(function(b) {
            return '<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;margin-bottom:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:6px;">' +
                '<div><strong>' + b.name + '</strong>' + (b.description ? '<div style="font-size:12px;color:#aaa;margin-top:4px;">' + b.description + '</div>' : '') + '</div>' +
                '<button onclick="applyBranch(\'' + b.id + '\')" style="padding:6px 14px;background:var(--accent,#3498db);color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:bold;">申请加入</button>' +
            '</div>';
        }).join('');
    } catch(e) {
...
function renderBranchSelector() {
    ...
    arrow.style.cssText = 'position:absolute;right:8px;top:50%;transform:translateY(-50%);color:rgba(255,255,255,0.4);font-size:10px;pointer-events:none;';
...
async function loadList() {
    try {
        ...
        list.forEach(function(c) {
            ...
            var deleteButton = userRole >= 1
                ? '<button class="tool-btn" onclick="delChar(event, \'' + c.id + '\')"><i class="fas fa-trash"></i></button>'
                : '';
