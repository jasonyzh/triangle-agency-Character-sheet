...
function goPlaza() {
    createTransition('访问办公室', 'office.html');
}
...
async function loadList() {
    try {
        ...
        list.forEach(function(c) {
            ...
            var plazaBtn = '<button class="tool-btn plaza-toggle ' + (c.plazaVisible ? 'plaza-on' : 'plaza-off') + '" onclick="togglePlaza(event, \'' + c.id + '\', this)" title="' + (c.plazaVisible ? '办公室展示中' : '未在办公室展示') + '"><i class="fas fa-globe"></i></button>';
...
async function togglePlaza(e, id, btn) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            showToast(data.plazaVisible ? '已在办公室展示' : '已关闭办公室展示', 'success');
        }
    } catch (err) {
        showToast('操作失败');
    }
