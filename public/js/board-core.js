function BoardCore(canvasEl, { editable, role, imageBaseUrl, onImageMove, onImageResize, onImageClick, onImageRightClick, onNpcLineDblClick, onNpcLineRightClick, onMapLineRightClick, onImageDblClick }) {
    ...
    this.addImage = function(data) {
        ...
        el.style.cssText = 'position:absolute;left:' + data.x + 'px;top:' + data.y + 'px;z-index:' + ((data.z || 0) + 2) + ';';
        ...
        img.src = this.imageBaseUrl + data.imageFile;
        img.draggable = false;
        ...
        if (this.editable) {
            ...
            handle.style.cssText = 'position:absolute;right:0;bottom:22px;width:14px;height:14px;background:rgba(192,57,43,0.6);cursor:nwse-resize;border-radius:2px;z-index:2;';
            ...
            el.addEventListener('pointerdown', (e) => {
                ...
                const onMove = (ev) => { el.style.left = (ol + ev.clientX - sX) + 'px'; el.style.top = (ot + ev.clientY - sY) + 'px'; self.drawAll(); };
                const onUp = (ev) => { document.removeEventListener('pointermove', onMove); document.removeEventListener('pointerup', onUp); if (Math.abs(ev.clientX - sX) < 3 && Math.abs(ev.clientY - sY) < 3) return; self.onImageMove(data.id, el.offsetLeft, el.offsetTop); };
    ...
    function nearestEdge(el, targetCX, targetCY) {
        ...
        const edges = [
            ...
            { x: l + iw / 2, y: t + ih, dist: Math.hypot(l + iw/2 - targetCX, t + ih - targetCY) },
            { x: l, y: t + ih / 2, dist: Math.hypot(l - targetCX, t + ih/2 - targetCY) },
            { x: l + iw, y: t + ih / 2, dist: Math.hypot(l + iw - targetCX, t + ih/2 - targetCY) }
        ];
    ...
    this.drawMapConnections = function() {
        ...
        (this.connections || []).forEach(conn => {
            ...
            const acy = elA.offsetTop + imgA.offsetHeight / 2;
            const bcx = elB.offsetLeft + imgB.offsetWidth / 2;
            const bcy = elB.offsetTop + imgB.offsetHeight / 2;
