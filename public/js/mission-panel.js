...
if (!missionId) { showMissionPicker(); }

/* 总经理终端：未带任务ID时选择激活任务（选定后跳转到带ID地址） */
async function showMissionPicker() {
    const wrap = document.createElement('div');
    wrap.id = 'mpPicker';
    wrap.className = 'mp-picker';
    wrap.innerHTML = '<div class="mp-picker-box"><div class="mp-picker-head"><b>选择任务</b><span>总经理终端 · 选择一个激活任务进入</span></div><div class="mp-picker-list" id="mpPickerList"><div class="mp-picker-loading">加载任务中…</div></div></div>';
    document.body.appendChild(wrap);
    try {
        const res = await fetch('/api/manager/missions?status=active', { headers: getAuthHeaders() });
        const list = await res.json();
        const arr = list || [];
        const box = document.getElementById('mpPickerList');
        if (!arr.length) { box.innerHTML = '<div class="mp-picker-empty">暂无进行中的任务</div>'; return; }
        box.innerHTML = arr.map(function (t) {
            return '<div class="mp-picker-item" data-id="' + t.id + '">'
                + '<b>' + (t.name || '未命名任务') + '</b>'
                + '<span class="mp-picker-meta">' + (t.mission_type === 'sweep' ? '清扫' : '收容') + ' · ' + (t.members ? t.members.length : 0) + ' 名特工</span></div>';
        }).join('');
        box.querySelectorAll('.mp-picker-item').forEach(function (it) {
            it.addEventListener('click', function () {
                location.href = 'mission-panel.html?missionId=' + encodeURIComponent(it.dataset.id);
            });
        });
    } catch (e) {
        const box = document.getElementById('mpPickerList');
        if (box) box.innerHTML = '<div class="mp-picker-empty">加载失败，请刷新重试</div>';
    }
}
...
function renderAgentRow(c, idx) {
    ...
    if (anoms.length) { html += '<div class="mp-card-list">'; anoms.forEach(a => {
        ...
        const s = a.succ ? stripHtmlText(a.succ).trim() : '', f = isPassive ? '' : (a.fail ? stripHtmlText(a.fail).trim() : '');
        html += '<div class="mp-card mp-card-anom' + (isPassive ? ' passive' : '') + '"' + (s || f || a.tdesc ? ' onclick="this.classList.toggle(\'expanded\')"' : '') + '>';
        html += '<div class="mp-card-head"><span class="mp-card-name">' + esc(a.name) + '</span>' + (isPassive ? '<span class="mp-card-passive"><i class="fas fa-shield-alt"></i> 被动</span>' : (a.trig ? '<span class="mp-card-trig">' + esc(a.trig) + '</span>' : '')) + '</div>';
        if (s || f || a.tdesc) { html += '<div class="mp-card-body">'; if (s) html += '<div class="mp-card-succ"><i class="fas ' + (isPassive ? 'fa-shield-alt' : 'fa-check-circle') + '"></i> ' + (isPassive ? '描述：' : '') + esc(s) + '</div>'; if (f) html += '<div class="mp-card-fail"><i class="fas fa-times-circle"></i> ' + esc(f) + '</div>'; if (a.tdesc) html += '<div class="mp-card-question"><i class="fas fa-question-circle"></i> ' + esc(a.tdesc) + '</div>'; html += '</div>'; }
        html += '</div>'; }); html += '</div>';
...
function goBack() { window.close(); if (!window.closed) window.location.href = 'desktop.html'; }
function goManagerEdit() { window.open('desktop.html', '_blank'); }
init();

/* 切换任务：回到无参地址 → 页内显示任务选择层 */
function goMissionPicker() {
    const old = document.getElementById('mpPicker');
    if (old) old.remove();
    location.href = 'mission-panel.html';
}
