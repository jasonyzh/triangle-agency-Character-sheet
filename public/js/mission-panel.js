...
function initSocket() {
    ...
    socket.on('connect', () => { console.log('Board socket connected'); });
}
...
function renderImageLibrary() {
    ...
    const html = !filtered.length
        ...
        : filtered.map(img => '<div class="lib-item" draggable="true" ondragstart="onLibDrag(event,\'' + img.id + '\',\'' + img.filename + '\',\'' + (img.original_name||img.filename).replace(/'/g,"\\'") + '\',\'' + img.category + '\')" ondblclick="renameLibImage(\'' + img.id + '\',\'' + (img.original_name||img.filename).replace(/'/g,"\\'") + '\')" oncontextmenu="event.preventDefault();showLibItemMenu(event,\'' + img.id + '\',\'' + (img.original_name||img.filename).replace(/'/g,"\\'") + '\')"><img src="/' + img.filename + '" title="双击改名｜右键删除"><span>' + (img.original_name||img.filename).substring(0, 10) + '</span></div>').join('');
    list.innerHTML = html;
}
...
async function initBoardView() {
    ...
    try {
        ...
        socket.off('board:image-move').on('board:image-move', function(d) { if (gmBoard) gmBoard.moveImage(d.imageId, d.x, d.y); });
        socket.off('board:image-resize').on('board:image-resize', function(d) { if (gmBoard) gmBoard.resizeImage(d.imageId, d.w, d.h); });
        ...
        socket.off('board:image-remove').on('board:image-remove', function(d) { if (gmBoard) gmBoard.removeImage(d.imageId); });
...
async function deleteBoardImage(imageId) {
    ...
    if (gmBoard) gmBoard.removeImage(imageId);
}
