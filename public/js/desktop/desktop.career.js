...
(function () {
  ...
  function updateCareerCard() {
    ...
    applyBgTheme(pcts.F, pcts.R, pcts.A);
  }

  /* 三轨主题背景：职能=默认红、现实=yellowbg、异常=bluebg；同分优先级 红 > 黄 > 蓝 */
  function applyBgTheme(f, r, a) {
    var theme = (f >= r && f >= a) ? 'red' : (r >= a ? 'yellow' : 'blue');
    document.body.classList.toggle('bg-blue', theme === 'blue');
    document.body.classList.toggle('bg-yellow', theme === 'yellow');
    var left = document.querySelector('.bg-left');
    var right = document.querySelector('.bg-right');
    if (!left || !right) return;
    var srcs = {
      red: ['img/desktopl.webp', 'img/desktopr.webp'],
      yellow: ['img/yellowbgl.svg', 'img/yellowbgr.svg'],
      blue: ['img/bluebgl.svg', 'img/bluebgr.svg']
    };
    var lSrc = srcs[theme][0], rSrc = srcs[theme][1];
    if (left.getAttribute('src') !== lSrc) left.setAttribute('src', lSrc);
    if (right.getAttribute('src') !== rSrc) right.setAttribute('src', rSrc);
  }
  window.DESKTOP.onCardReady(updateCareerCard);
  /* 供 desktop.js 在切换角色后刷新生涯卡片与背景主题 */
  DA.feats.career = { refresh: updateCareerCard };
