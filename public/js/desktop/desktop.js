(function () {
  ...
  /* 供应用窗口模块读取当前激活角色 */
  window.DESKTOP = { getActiveCharId: function () { return activeCharId; }, getCardData: function () { return activeData(); }, authHeaders: function () { return authHeaders; }, getToken: function () { return token; }, bringToFront: function (win) { bringToFrontEl(win); }, registerWin: function (win) { if (win && winOrder.indexOf(win) < 0) winOrder.push(win); }, makeDraggable: function (win, onlyWindowed) { makeDraggable(win, onlyWindowed); }, onCardReady: function (cb) { if (charsLoaded) cb(); else cardCbs.push(cb); }, refreshChar: function () { loadProfile(); }, getCurChar: function () { return curChar(); }, setBranch: function (id) { saveBranch(id); } };
