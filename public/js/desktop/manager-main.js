...
import { loadBranchApplications, reviewApplication, initMCharSideSwitch, loadBranchProfilePane } from '../manager/branches.js?v=4';
...
if (authorized) {
  initMCharSideSwitch();   /* 特工档案窗口侧边功能切换（特工管理/分部管理） */
  ...
  branchesReady.then(function () {
    ...
    loadMissions();
    loadInbox();
    loadBranchProfilePane();   /* 预填分部管理面板（分部下拉） */
