...
(function () {
  ...
  function openMailView(m) {
    ...
    /* 邮件中提到且当前角色已解锁的高墙文件 → 内容下方加直达查看按钮 */
    permittedDocCodes(function (codes) {
      if (!codes.length) return;
      /* 分词精确匹配：A1 不会误配 A11 */
      const tokens = (c.textContent.toUpperCase().match(/[A-Z0-9]+/g)) || [];
      const found = [];
      tokens.forEach(function (tk) {
        if (codes.indexOf(tk) >= 0 && found.indexOf(tk) < 0) found.push(tk);
      });
      if (!found.length) return;
      const bar = document.createElement('div');
      bar.className = 'mv-docs';
      found.forEach(function (code) {
        const b = document.createElement('button');
        b.type = 'button';
        b.className = 'mv-doc-btn';
        b.textContent = '查看 ' + code + ' 高墙文件';
        b.addEventListener('click', function () { openDocFromMail(code); });
        bar.appendChild(b);
      });
      c.appendChild(bar);
    });
  ...
  /* ========== 邮件提及的已解锁高墙文件：直达查看 ========== */
  var permDocsCache = { cid: null, codes: null };
  function permittedDocCodes(cb) {
    const cidVal = cid();
    if (!cidVal) { cb([]); return; }
    if (permDocsCache.cid === cidVal && permDocsCache.codes) { cb(permDocsCache.codes); return; }
    fetch('/api/documents/list?charId=' + encodeURIComponent(cidVal), { headers: authH() })
      .then(function (r) { return r.ok ? r.json() : []; })
      .then(function (list) {
        const codes = (list || []).map(function (f) { return String(f.filename || '').replace(/\.md$/i, '').toUpperCase(); });
        permDocsCache = { cid: cidVal, codes: codes };
        cb(codes);
      })
      .catch(function () { cb([]); });
  }
  function openDocFromMail(code) {
    $('mvSubject').textContent = code + ' 高墙文件';
    const c = $('mvContent');
    c.innerHTML = '<div style="text-align:center;color:#b3a8a8;padding:30px 0;"><i class="fas fa-circle-notch fa-spin"></i> 加载中...</div>';
    fetch('/api/documents/read/' + encodeURIComponent(code) + '.md', { headers: authH() })
      .then(function (r) { return r.json(); })
      .then(function (d) {
        if (!d.content) throw new Error('empty');
        const md = window.marked;
        c.innerHTML = md ? (typeof md.parse === 'function' ? md.parse(d.content) : md(d.content)) : esc(d.content);
        c.style.whiteSpace = 'normal';
      })
      .catch(function () { c.innerHTML = '<div style="text-align:center;color:#b3a8a8;padding:30px 0;">读取失败或未解锁</div>'; });
  }
