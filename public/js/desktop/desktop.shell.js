...
(function () {
  ...
  var APPWINS = { docs: 'winDocs', board: 'winBoard', shop: 'winShop', highwall: 'winHW', 'siphon-shop': 'winSiphonShop', office: 'winOffice', history: 'winHistory', 'branch-join': 'winBranchJoin',
  ...
  var APP_META = {
    ...
    history: { title: '出勤记录', ico: '<svg viewBox="0 0 48 48"><circle cx="24" cy="25" r="16" fill="none" stroke="#fff" stroke-width="3.5"/><path d="M24 16 v9 l7 5" fill="none" stroke="#fff" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M17 6 L10 12 M31 6 L38 12" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"/></svg>' },
    'branch-join': { title: '加入分部', ico: '<svg viewBox="0 0 48 48"><path d="M9 42 V14 a2 2 0 0 1 2 -2 h12 a2 2 0 0 1 2 2 v28 Z" fill="#fff"/><path d="M25 42 V22 a2 2 0 0 1 2 -2 h11 a2 2 0 0 1 2 2 v20 Z" fill="#fff" opacity=".82"/><circle cx="36" cy="12" r="8" fill="#e8907f"/><path d="M32.8 12 h6.4 M36 8.8 v6.4" stroke="#fff" stroke-width="2.4" stroke-linecap="round"/></svg>' },
  ...
  function openApp(key) {
    ...
    if (!w.dataset.started) {
      ...
      /* 已启动的窗口再次打开/还原时刷新数据（仅显式提供 reload 的模块响应） */
      var rf = DA.feats[key];
      if (rf && rf.reload) rf.reload();
    }
    /* 管理台窗口需经理权限；系统设置仅超管（图标已隐藏，这里防绕过） */
    if (key.slice(0, 2) === 'm-' && parseInt(localStorage.getItem('ta_role') || '0', 10) < 1) {
      showToast('需要经理权限');
      return;
    }
  ...
  ['winDocs', 'winBoard', 'winShop', 'winHW', 'winOffice', 'winHistory', 'winBranchJoin'].forEach(function (id) {
    window.DESKTOP.makeDraggable($(id), true);
  });
  ['winDocs', 'winBoard', 'winShop', 'winHW', 'winOffice', 'winHistory', 'winBranchJoin', 'winCareer', 'winAnom', 'winReal', 'winItem', 'winMail'].forEach(function (id) {
  ...
  /* 会话切换（同页登出/换号登录）：清掉上个账号的管理台窗口、任务栏与图标点亮，按新角色重算 */
  function resetPrivilegedUi() {
    var role = parseInt(localStorage.getItem('ta_role') || '0', 10);
    Array.prototype.forEach.call(document.querySelectorAll('.dico.mgr-only, .dico.admin-only'), function (d) {
      d.classList.remove('granted');
      if ((d.classList.contains('mgr-only') && role >= 1) || (d.classList.contains('admin-only') && role >= 2)) d.classList.add('granted');
    });
    Object.keys(APPWINS).forEach(function (k) {
      if (k.slice(0, 2) !== 'm-') return;
      var w = $(APPWINS[k]);
      w.classList.remove('show');
      removeTaskBtn(k);
      delete w.dataset.started;
    });
    syncTaskStates();
    persistApps();
  }
  window.DESKTOP.resetPrivilegedUi = resetPrivilegedUi;
  /* ========== 恢复上次打开的应用（刷新不关闭；管理台窗口仅经理/超管恢复） ========== */
  (function restoreApps() {
    ...
    var role = parseInt(localStorage.getItem('ta_role') || '0', 10);
    saved.open.forEach(function (k) {
      if (!APPWINS[k]) return;
      if (k.slice(0, 2) === 'm-' && role < 1) return;   /* 低权限账号不恢复管理台窗口 */
      if (k === 'm-admin' && role < 2) return;
      addTaskBtn(k);
    });
    ...
    visList.forEach(function (vis) {
      ...
      if (vis.slice(0, 2) === 'm-' && role < 1) return;   /* 低权限账号不恢复管理台窗口 */
      if (vis === 'm-admin' && role < 2) return;
      var w = $(APPWINS[vis]);
      w.dataset.started = '1';
      w.classList.add('show');
