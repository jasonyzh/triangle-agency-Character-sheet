...
(function () {
  ...
  /* ================= 屏幕切换箭头（主屏两侧 + 侧屏返回主屏） =================
     主屏左箭头 → 负一屏、右箭头 → 分部选择屏；负一屏/分部选择屏各有一个返回主屏按钮。
     可见性由 body[data-ma] 驱动（desktop.css），点击直接切屏，与滑动手势等价。 */
  var navChevron = function (dir) {
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="' +
      (dir === 'left' ? 'M14.5 5 L7.5 12 l7 7' : 'M9.5 5 L16.5 12 l-7 7') + '"/></svg>';
  };
  function makeNavBtn(id, title, dir) {
    var b = document.createElement('button');
    b.id = id;
    b.className = 'ma-nav-btn ' + (dir === 'left' ? 'ma-nav-l' : 'ma-nav-r');
    b.title = title;
    b.innerHTML = navChevron(dir);
    return b;
  }
  var navLeft = makeNavBtn('maNavLeft', '负一屏', 'left');
  var navRight = makeNavBtn('maNavRight', '分部选择', 'right');
  var navMinusHome = makeNavBtn('maNavMinusHome', '返回主屏', 'right');
  var navProfileHome = makeNavBtn('maNavProfileHome', '返回主屏', 'left');
  navLeft.addEventListener('click', function () { setScreen('minus'); });
  navRight.addEventListener('click', function () { setScreen('profile'); });
  navMinusHome.addEventListener('click', function () { setScreen('home'); });
  navProfileHome.addEventListener('click', function () { setScreen('home'); });
  [navLeft, navRight, navMinusHome, navProfileHome].forEach(function (b) { document.body.appendChild(b); });
  ...
  /* ================= 手机端禁用系统/浏览器「返回」 =================
     需求：手机端（≤860px）完全禁用返回功能——按 Android 系统返回键或触发浏览器
     返回手势（iOS 边缘滑动等）时不再退出/回退页面，应用始终停留在当前桌面。
     实现：历史栈顶部压入哨兵状态，popstate（来自返回）时立即补压，任何返回动作
     都被抵消；popstate 触发时动态判断当前宽度，切回桌面宽度（如平板横屏）后
     返回行为自动恢复正常。本页全程不用 history 导航，不影响其它逻辑。 */
  var mobileMedia = window.matchMedia('(max-width: 860px)');
  var LOCK = { taLock: 1 };
  if (mobileMedia.matches) {
    history.replaceState(LOCK, '');
    history.pushState(LOCK, '');
  }
  window.addEventListener('popstate', function () {
    if (mobileMedia.matches) history.pushState(LOCK, '');
  });
})();
