...
export function renderHighwallFiles(container) {
    ...
    container.innerHTML = '<div class="hw-folder">' + S.mailData.highwallFiles.map(f => `
        <div class="hw-file" onclick="openHighwallDoc('${f.filename}')">
            <i class="fas fa-file-alt"></i>
...
export function openHighwallFromMail(filename) {
    closeMailReader();
    setTimeout(() => openHighwallDoc(filename), 300);
}

// 弹窗显示高墙文件内容（复用邮件阅读器样式，不跳转页面）
export async function openHighwallDoc(filename) {
    const overlay = document.getElementById('mail-reader-overlay');
    const subjectEl = document.getElementById('mail-reader-subject');
    const senderEl = document.getElementById('mail-reader-sender');
    const timeEl = document.getElementById('mail-reader-time');
    ...
    const eyeContainer = document.querySelector('.mail-eye-container');
    if (!overlay) return;
    const code = filename.replace(/\.md$/i, '');
    subjectEl.textContent = '高墙文件 ' + code;
    senderEl.innerHTML = '<span class="os-badge">OS</span> 系统通知';
    timeEl.innerHTML = '<i class="fas fa-file-shield"></i> 高墙文件';
    bodyEl.innerHTML = '<div style="text-align:center;color:#6090b0;padding:30px;"><i class="fas fa-circle-notch fa-spin"></i> 加载中...</div>';
    actionsEl.innerHTML = '<button class="mail-reader-btn secondary" onclick="closeMailReader()"><i class="fas fa-times"></i> 关闭</button>';
    eyeContainer.classList.add('mail-eye-blink');
    setTimeout(() => eyeContainer.classList.remove('mail-eye-blink'), 300);
    overlay.classList.add('active');
    try {
        const res = await fetch('/api/documents/read/' + encodeURIComponent(filename), { headers: { 'Authorization': 'Bearer ' + S.token } });
        const data = await res.json();
        if (!res.ok) {
            bodyEl.innerHTML = '<div style="text-align:center;color:#e74c3c;padding:30px;"><i class="fas fa-lock"></i><p style="margin-top:10px;">' + (data.error || '无权访问此文件') + '</p></div>';
            return;
        }
        bodyEl.innerHTML = window.marked ? marked.parse(data.content) : '<pre style="white-space:pre-wrap;">' + escapeHtmlMail(data.content || '') + '</pre>';
    } catch (e) {
        bodyEl.innerHTML = '<div style="text-align:center;color:#e74c3c;padding:30px;">加载失败</div>';
    }
}
