...
export function addAnom(d = null, prepend = false, skipCheck = false) {
    ...
    const div = document.createElement('div'); div.className = 'card bd-anom anom-card'; div.innerHTML = `<button class="anom-card-edit-btn" onclick="openAnomCardEdit(this.closest('.anom-card'))"><i class="fas fa-pen"></i></button>${createDelBtn('anom')}<input type="hidden" class="f-name"><input type="hidden" class="f-trig"><input type="hidden" class="f-qual"><input type="hidden" class="f-passive" value="0"><input type="hidden" class="f-sub-on" value="0"><input type="hidden" class="f-sub"><input type="hidden" class="f-list-on" value="0"><input type="hidden" class="f-list-name"><input type="hidden" class="f-list" value="[]"><input type="hidden" class="f-tdesc"><input type="hidden" class="f-t1"><input type="hidden" class="f-t1-val"><input type="hidden" class="f-t2"><input type="hidden" class="f-t2-val"><input type="hidden" class="f-t3"><input type="hidden" class="f-t3-val"><input type="hidden" class="f-p1n" value="3"><input type="hidden" class="f-p2n" value="3"><input type="hidden" class="f-p3n" value="3"><input type="checkbox" class="f-chk" style="display:none"><div class="sq-dots d1" style="display:none"><div class="sq-dot"></div><div class="sq-dot"></div><div class="sq-dot"></div></div><div class="sq-dots d2" style="display:none"><div class="sq-dot"></div><div class="sq-dot"></div><div class="sq-dot"></div></div><div class="sq-dots d3" style="display:none"><div class="sq-dot"></div><div class="sq-dot"></div><div class="sq-dot"></div></div><div class="rich-editor f-succ" contenteditable="true" style="display:none"></div><div class="rich-editor f-fail" contenteditable="true" style="display:none"></div><div class="anom-title-bar"><div class="anom-title-row"><span class="anom-disp-name"></span><span class="anom-field-sep">|</span><span class="anom-disp-trig"></span></div><div class="anom-disp-qual-row"><span class="anom-disp-qual"></span></div></div><div class="anom-body"><div class="anom-result-row"><div class="anom-result succ-section"><div class="anom-result-label c-anom">${anomIcon('check')}<span>成功时</span></div><div class="anom-disp-succ"></div></div><div class="anom-result anom-sub-sec" style="display:none"><div class="anom-sec-label anom-sec-sub">${anomIcon('wave')}<span>三重升华时</span></div><div class="anom-disp-sub"></div></div><div class="anom-result anom-list-sec" style="display:none"><div class="anom-result-label c-anom">${anomIcon('star')}<span></span></div><div class="anom-timeline"></div></div><div class="anom-result fail-section"><div class="anom-result-label" style="color:#c0392b">${anomIcon('x')}<span>失败时</span></div><div class="anom-disp-fail"></div></div></div></div><div class="anom-question-section"><div class="anom-disp-question"></div><div class="anom-disp-answers"><div class="anom-disp-a1"></div><div class="anom-disp-a2"></div><div class="anom-disp-a3"></div></div></div>`;
...
export function fillAnom(div, d) {
    ...
    div.querySelector('.f-trig').value = d.trig || '';
    div.querySelector('.f-qual').value = d.qual || '';
    div.querySelector('.f-passive').value = d.passive ? '1' : '0';
    div.querySelector('.f-sub-on').value = d.subOn ? '1' : '0';
    div.querySelector('.f-sub').value = d.sub || '';
    div.querySelector('.f-list-on').value = d.listOn ? '1' : '0';
    div.querySelector('.f-list-name').value = d.listName || '';
    div.querySelector('.f-list').value = JSON.stringify(d.list || []);
...
export function syncAnomDisplay(card) {
    ...
    if (succLabel) succLabel.innerHTML = passive ? '<i class="fas fa-shield-alt"></i> 描述' : anomIcon('check') + '<span>成功时</span>';
    if (failSection) failSection.style.display = passive ? 'none' : '';

    // 三重升华 / 列表（勾选启用后才显示）
    const subOn = card.querySelector('.f-sub-on').value === '1';
    const subHtml = card.querySelector('.f-sub').value || '';
    const subSec = card.querySelector('.anom-sub-sec');
    const subShown = subOn && subHtml.trim();
    if (subSec) {
        subSec.style.display = subShown ? '' : 'none';
        if (subShown) card.querySelector('.anom-disp-sub').innerHTML = subHtml;
    }
    // 无三重升华时失败时上移补位（CSS 网格）
    const resultRowEl = card.querySelector('.anom-result-row');
    if (resultRowEl) resultRowEl.classList.toggle('no-sub', !subShown);
    let listArr = [];
    try { listArr = JSON.parse(card.querySelector('.f-list').value || '[]'); } catch (e) {}
    const listOn = card.querySelector('.f-list-on').value === '1';
    const listName = card.querySelector('.f-list-name').value.trim() || '列表';
    const listSec = card.querySelector('.anom-list-sec');
    if (listSec) {
        const nameSpan = listSec.querySelector('.anom-result-label span');
        if (nameSpan) nameSpan.textContent = listName;
        // 勾选启用即显示（允许没有子项）；只有一个子项时不显示时间线，文本靠最左
        listSec.style.display = listOn ? '' : 'none';
        const tlEl = card.querySelector('.anom-timeline');
        if (listArr.length === 1) {
            tlEl.classList.add('single');
            tlEl.innerHTML = '<div class="tl-text">' + esc(listArr[0]) + '</div>';
        } else {
            tlEl.classList.remove('single');
            tlEl.innerHTML = listArr.map(r => '<div class="tl-row"><span class="tl-node"></span><span class="tl-text">' + esc(r) + '</span></div>').join('');
        }
    }
    ...
    if (dt) dt.innerHTML = trig ? '<span class="anom-disp-tag c-anom"><i class="fas fa-star"></i> 资质</span> ' + esc(trig) : '<span class="anom-empty">无资质</span>';
    const dq = card.querySelector('.anom-disp-qual');
    if (dq) dq.innerHTML = qual ? '<span class="anom-disp-tag c-anom"><i class="fas fa-bolt"></i> 触发</span> ' + esc(qual) : '';
...
export function updateAnomEditPassiveUI(isPassive) {
    ...
    if (succLabel) succLabel.innerHTML = isPassive ? '<i class="fas fa-shield-alt"></i> 描述' : anomIcon('check') + '<span>成功时</span>';
...
// 编辑弹窗：三重升华/列表模块开关（勾选后才显示编辑区）
export function anomEditToggleModule(which, on) {
    const modal = document.getElementById('anomEditModal');
    if (!modal) return;
    if (which === 'sub') {
        const ed = modal.querySelector('.anom-edit-sub');
        if (ed) ed.style.display = on ? '' : 'none';
    } else {
        const wrap = document.getElementById('anomEditListWrap');
        if (wrap) wrap.style.display = on ? '' : 'none';
    }
}

// 编辑弹窗：添加一个列表行（时间线节点 + 输入框 + 删除）
export function anomEditAddListRow(val) {
    const wrap = document.getElementById('anomEditListRows');
    if (!wrap) return;
...
function readEditListRows() {
    return Array.from(document.querySelectorAll('#anomEditListRows input')).map(i => i.value.trim()).filter(v => v);
}
...
export function openAnomCardEdit(cardEl) {
    ...
    // 三重升华 / 列表
    const subOn = cardEl.querySelector('.f-sub-on').value === '1';
    q('.anom-edit-sub-on').checked = subOn;
    q('.anom-edit-sub').innerHTML = cardEl.querySelector('.f-sub').value || '';
    q('.anom-edit-sub').style.display = subOn ? '' : 'none';
    const listOn = cardEl.querySelector('.f-list-on').value === '1';
    q('.anom-edit-list-on').checked = listOn;
    document.getElementById('anomEditListWrap').style.display = listOn ? '' : 'none';
    q('.anom-edit-list-name').value = cardEl.querySelector('.f-list-name').value;
    const rowsWrap = document.getElementById('anomEditListRows');
    rowsWrap.innerHTML = '';
    let listArr = [];
    try { listArr = JSON.parse(cardEl.querySelector('.f-list').value || '[]'); } catch (e) {}
    if (!listArr.length) listArr = [''];
    listArr.forEach(v => anomEditAddListRow(v));
...
function applyAnomImport(idx) {
    ...
    q('.anom-edit-passive').checked = passive;
    updateAnomEditPassiveUI(passive);
    // 三重升华
    const subOn = !!a.subOn;
    q('.anom-edit-sub-on').checked = subOn;
    q('.anom-edit-sub').innerHTML = a.sub || '';
    q('.anom-edit-sub').style.display = subOn ? '' : 'none';
    // 列表
    const listOn = !!a.listOn;
    q('.anom-edit-list-on').checked = listOn;
    document.getElementById('anomEditListWrap').style.display = listOn ? '' : 'none';
    q('.anom-edit-list-name').value = a.listName || '';
    const rowsWrap = document.getElementById('anomEditListRows');
    rowsWrap.innerHTML = '';
    let rows = Array.isArray(a.list) ? a.list.slice() : [];
    if (!rows.length) rows = [''];
    rows.forEach(v => anomEditAddListRow(v));
...
export function saveAnomCardEdit() {
    ...
    // 三重升华 / 列表
    card.querySelector('.f-sub-on').value = q('.anom-edit-sub-on').checked ? '1' : '0';
    card.querySelector('.f-sub').value = q('.anom-edit-sub').innerHTML || '';
    card.querySelector('.f-list-on').value = q('.anom-edit-list-on').checked ? '1' : '0';
    card.querySelector('.f-list-name').value = q('.anom-edit-list-name').value.trim();
    card.querySelector('.f-list').value = JSON.stringify(readEditListRows());
