...
import { loadConfigData, triggerAutoSave, gatherData, populateData, saveData, exportOffline, initAvatarUpload } from './api.js';
...
import { addAnom, deleteCard, updateSlotButtons, openAnomCardEdit, closeAnomCardEdit, saveAnomCardEdit, openAnomWindow, closeAnomWindow, initAnomDrag, handleBonusChange, resetBonus, handleAnomPassiveChange, openAnomImportModal, closeAnomImportModal, filterAnomImport, anomEditToggleModule, anomEditAddListRow } from './anomaly.js';
...
import { openMailModal, closeMailModal, switchMailTab, renderMailContent, renderInbox, renderSentMail, renderCompose, loadMailbox, deleteMail, renderHighwallFiles, openHighwallFile, openMailReader, closeMailReader, openHighwallFromMail, openHighwallDoc, openSentMailReader, openMessage, sendContainment, sendReport, closeSuccessModal, selectOutboxOption, addScatteringRow, addObjectiveRow } from './mail.js';
...
window.handleAnomPassiveChange = handleAnomPassiveChange;
window.openAnomImportModal = openAnomImportModal;
window.closeAnomImportModal = closeAnomImportModal;
window.filterAnomImport = filterAnomImport;
window.anomEditToggleModule = anomEditToggleModule;
window.anomEditAddListRow = anomEditAddListRow;
...
window.openHighwallDoc = openHighwallDoc;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    loadMailbox().then(markGrantedTrackCells);
    checkDestructionAccess();

    if (!S.playerSocket) {
        S.playerSocket = io({ auth: { token: S.token } });
    }
    if (!S.playerSocket.__mailBound) {
        S.playerSocket.__mailBound = true;
        S.playerSocket.on('mail:new', function () {
            setTimeout(function () {
                window.loadMailbox();
                var mailBtn = document.querySelector('.top-mail-btn');
                if (mailBtn) mailBtn.classList.add('has-unread');
            }, 500);
        });
    }
    ...
    if (offlineDataEl && offlineDataEl.textContent.trim().length > 2) {
        ...
        try {
            ...
            if (data.isArchived && !S.isReadOnly) {
                S.isReadOnly = true;
...
function goItems() {
    ...
    if (overlay) {
        const textContent = overlay.querySelector('.loader-text');
        const textNode = textContent ? textContent.childNodes[0] : null;
        if (textNode) textNode.textContent = '访问申领物仓库';
        overlay.classList.add('active');
    }
    setTimeout(() => { window.location.href = 'items.html?charId=' + S.charId; }, 800);
}
window.goItems = goItems;
...
function toggleTrackEdit() {
    ...
    document.body.classList.toggle('track-editing', trackEditMode);
    const btn = document.getElementById('trackEditBtn');
    if (btn) {
        btn.classList.toggle('active', trackEditMode);
        btn.innerHTML = trackEditMode ? '<i class="fas fa-check"></i>' : '<i class="fas fa-pen"></i>';
    }
}
window.toggleTrackEdit = toggleTrackEdit;

// 标记已授权高墙文件的格子（加 granted 类，显示可点击样式）
function markGrantedTrackCells() {
    const granted = (S.mailData.highwallFiles || []).map(f => (f.filename || '').toLowerCase().replace(/\.md$/, ''));
    document.querySelectorAll('.p-cell span').forEach(span => {
        const code = (span.textContent || '').trim().toLowerCase();
        const cell = span.closest('.p-cell');
        if (cell) {
            if (code && granted.includes(code)) cell.classList.add('granted');
            else cell.classList.remove('granted');
        }
    });
}
window.markGrantedTrackCells = markGrantedTrackCells;

// 非编辑模式：点击格子弹窗显示高墙文件内容（复用邮件阅读器样式）
async function openTrackDoc(code) {
    window.openHighwallDoc(code + '.md');
}
function closeTrackDoc() {
    const modal = document.getElementById('trackDocModal');
    if (modal) modal.style.display = 'none';
}
window.openTrackDoc = openTrackDoc;
window.closeTrackDoc = closeTrackDoc;

document.querySelectorAll('.p-cell').forEach(c => {
    c.addEventListener('click', () => {
        if (S.isReadOnly) return;
        // 非编辑模式：有标签的格子（如 A3）弹窗显示对应高墙文件（需已授权）
        if (!trackEditMode) {
            const span = c.querySelector('span');
            if (span && span.textContent.trim()) {
                const code = span.textContent.trim();
                const granted = (S.mailData.highwallFiles || []).some(f => f.filename && f.filename.toLowerCase() === (code + '.md').toLowerCase());
                if (!granted) return; // 未授权，点击无反应
                openTrackDoc(code);
            }
            return;
        }
