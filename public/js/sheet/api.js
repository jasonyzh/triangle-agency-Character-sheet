...
export function gatherData() {
    ...
    document.querySelectorAll('#list-anom .card').forEach(c => { d.anoms.push({ name: c.querySelector('.f-name').value, trig: c.querySelector('.f-qual').value, qual: c.querySelector('.f-trig').value, succ: c.querySelector('.f-succ').innerHTML, fail: c.querySelector('.f-fail').innerHTML, passive: c.querySelector('.f-passive').value === '1', chk: c.querySelector('.f-chk').checked, tdesc: c.querySelector('.f-tdesc').value, t1: c.querySelector('.f-t1').value, t1v: c.querySelector('.f-t1-val').value, t2: c.querySelector('.f-t2').value, t2v: c.querySelector('.f-t2-val').value, t3: c.querySelector('.f-t3').value, t3v: c.querySelector('.f-t3-val').value, p1: Array.from(c.querySelectorAll('.d1 .sq-dot')).map(e => e.classList.contains('active')), p2: Array.from(c.querySelectorAll('.d2 .sq-dot')).map(e => e.classList.contains('active')), p3: Array.from(c.querySelectorAll('.d3 .sq-dot')).map(e => e.classList.contains('active')) }); });
