...
export function gatherData() {
    ...
    document.querySelectorAll('#list-anom .card').forEach(c => { d.anoms.push({ name: c.querySelector('.f-name').value, trig: c.querySelector('.f-trig').value, qual: c.querySelector('.f-qual').value, succ: c.querySelector('.f-succ').innerHTML, fail: c.querySelector('.f-fail').innerHTML, passive: c.querySelector('.f-passive').value === '1', subOn: c.querySelector('.f-sub-on').value === '1', sub: c.querySelector('.f-sub').value, listOn: c.querySelector('.f-list-on').value === '1', listName: c.querySelector('.f-list-name').value, list: (function () { try { return JSON.parse(c.querySelector('.f-list').value || '[]'); } catch (e) { return []; } })(), chk: c.querySelector('.f-chk').checked, tdesc: c.querySelector('.f-tdesc').value, t1: c.querySelector('.f-t1').value, t1v: c.querySelector('.f-t1-val').value, t2: c.querySelector('.f-t2').value, t2v: c.querySelector('.f-t2-val').value, t3: c.querySelector('.f-t3').value, t3v: c.querySelector('.f-t3-val').value, p1: Array.from(c.querySelectorAll('.d1 .sq-dot')).map(e => e.classList.contains('active')), p2: Array.from(c.querySelectorAll('.d2 .sq-dot')).map(e => e.classList.contains('active')), p3: Array.from(c.querySelectorAll('.d3 .sq-dot')).map(e => e.classList.contains('active')) }); });
    ...
    // 头像（从隐藏字段读取，保持已有值或空）
    const avHidden = document.getElementById('pAvatarHidden');
    d.pAvatar = avHidden ? avHidden.value : '';
    return d;
}
...
export function populateData(d) {
    ...
    // 头像填充
    applyAvatar(d.pAvatar, d.pName, d._canEdit);
}

// 头像显示 + 上传绑定（canEdit 控制是否可点）
function applyAvatar(pAvatar, name, canEdit) {
    let hidden = document.getElementById('pAvatarHidden');
    if (!hidden) { hidden = document.createElement('input'); hidden.type = 'hidden'; hidden.id = 'pAvatarHidden'; document.body.appendChild(hidden); }
    hidden.value = pAvatar || '';

    const img = document.getElementById('charAvatar');
    const placeholder = document.getElementById('charAvatarPlaceholder');
    const box = document.getElementById('charAvatarBox');
    ...
    // 仅本人可上传
    if (canEdit && !S.isReadOnly) {
        box.classList.add('editable');
        box.onclick = () => document.getElementById('avatarFileInput').click();
    } else {
...
export function closeAvatarCrop() {
    const modal = document.getElementById('avatarCropModal');
    if (modal) modal.classList.remove('show');
    if (avatarCropper) { avatarCropper.destroy(); avatarCropper = null; }
}

export async function confirmAvatarCrop() {
    ...
    canvas.toBlob(async (blob) => {
        ...
        try {
            const res = await fetch(`/api/character/${S.charId}/avatar`, {
                method: 'POST',
            ...
            if (res.ok && data.success) {
                applyAvatar(data.avatar, document.getElementById('pName')?.value, true);
                closeAvatarCrop();
