...
// 导入 JSON 批量创建异常能力模板
// 格式：[{name, trig, qual, succ, fail, passive, tdesc, t1, t1v, t2, t2v, docFilename}]
async function importAnomalyJson(e) {
    ...
    if (!confirm('确定导入 ' + items.length + ' 条异常能力模板？同名模板将跳过。')) { e.target.value = ''; return; }

    // 已有模板名（跳过重名）
    const existing = new Set(S.anomalyTemplates.map(t => t.name));
    let created = 0, skipped = 0, failed = 0;
    for (const it of items) {
        if (!it.name || typeof it.name !== 'string') { failed++; continue; }
        if (existing.has(it.name)) { skipped++; continue; }
        const body = {
            branchId: S.currentBranchId,
            name: it.name,
            trig: it.trig || '',
        ...
        try {
            const res = await fetch('/api/anomaly-templates', { method: 'POST', headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            const data = await res.json();
            if (data.success) { created++; existing.add(it.name); } else { failed++; }
        } catch (err) { failed++; }
    }
    e.target.value = '';
    showToast('导入完成：新增 ' + created + '，跳过 ' + skipped + '，失败 ' + failed, created > 0 ? 'success' : 'error');
    await loadAnomalyTemplates();
}
