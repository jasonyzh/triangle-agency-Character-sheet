...
function renderCharacters(chars) {
    ...
    container.innerHTML = chars.map(c => {
        ...
        return `
        <div class="char-card${c.isArchived ? ' archived' : ''}">
            <div class="card-header" onclick="openSheet('${c.id}')">
                <div class="card-header-actions">
                    <button class="card-top-btn btn-plaza-toggle ${c.plazaHidden ? '' : 'active'}" onclick="event.stopPropagation(); togglePlazaVisibility('${c.id}', ${!c.plazaHidden})" title="${c.plazaHidden ? '在办公室中展示' : '在办公室中隐藏'}">
                        <i class="fas fa-globe"></i>
                    </button>
                    <button class="card-top-btn btn-archive-toggle ${c.isArchived ? 'active' : ''}" onclick="event.stopPropagation(); toggleArchive('${c.id}', ${!c.isArchived})" title="${c.isArchived ? '取消归档' : '归档角色'}">
                        <i class="fas fa-archive"></i>
                    </button>
                </div>
                <div class="card-name">${c.name}${c.isArchived ? ' <span class="archived-tag">已归档</span>' : ''}</div>
            ...
            <div class="card-footer">
                <button class="btn-card-action btn-op" onclick="event.stopPropagation(); openRecordModal('${c.id}', '${safeCharName}')" title="嘉奖/申诫">
                    <i class="fas fa-medal"></i>
                </button>
                ${S.role >= 2 ? `<button class="btn-card-action btn-op" onclick="event.stopPropagation(); openRequisitionPermModal(${c.ownerId})" title="权限物品授权">
                    <i class="fas fa-gift"></i>
                </button>` : ''}
                <button class="btn-card-action btn-op" onclick="event.stopPropagation(); openDocModal('${c.id}', '${safeCharName}')" title="高墙授权">
                    <i class="fas fa-file-shield"></i>
                </button>
                <button class="btn-card-action btn-op" onclick="event.stopPropagation(); openGrantAnomalyModal('${c.id}', '${safeCharName}')" title="赋予异常能力">
                    <i class="fas fa-bolt"></i>
                </button>
                <button class="btn-card-action btn-op" onclick="event.stopPropagation(); openGrantNpcModal('${c.id}', '${safeCharName}')" title="添加关系">
                    <i class="fas fa-heart"></i>
                </button>
                <button class="btn-card-action btn-op" onclick="event.stopPropagation(); openSlotModal('${c.id}', '${safeCharName}')" title="槽位管理">
                    <i class="fas fa-unlock-alt"></i>
                </button>
                <button class="btn-card-action btn-op" onclick="event.stopPropagation(); printSheet('${c.id}')" title="打印角色卡">
                    <i class="fas fa-print"></i>
                </button>
                <button class="btn-card-action btn-op-danger" onclick="event.stopPropagation(); deleteCharacter('${c.id}', '${safeCharName}')" title="彻底删除角色">
                    <i class="fas fa-trash-alt"></i>
                </button>
                <button class="btn-card-action btn-op-edit" onclick="openSheet('${c.id}')" title="编辑">
...
function printSheet(id) {
    window.open(`print-sheet.html?id=${id}`, '_blank');
}
...
async function openRecordModal(charId, charName) {
    ...
    document.getElementById('tabMvp').className = 'record-tab';
    document.getElementById('tabWatch').className = 'record-tab';
    updateAddButton();
...
function switchRecordTab(tab) {
    ...
    const tabs = ['Reward', 'Reprimand', 'Mvp', 'Watch'];
    tabs.forEach(t => {
        const el = document.getElementById('tab' + t);
        if (el) el.className = 'record-tab';
    });

    const activeEl = document.getElementById('tab' + tab.charAt(0).toUpperCase() + tab.slice(1));
    if (activeEl) {
        if (tab === 'reward' || tab === 'mvp') activeEl.className = 'record-tab active-reward';
        else activeEl.className = 'record-tab active-reprimand';
    }
...
function updateAddButton() {
    ...
    const labels = {
        reward: { add: '添加嘉奖', deduct: '扣除嘉奖', addClass: 'reward', deductBg: '#e67e22' },
        reprimand: { add: '添加申诫', deduct: '扣除申诫', addClass: 'reprimand', deductBg: '#c0392b' },
        mvp: { add: '添加MVP', deduct: null, addClass: 'reward', deductBg: '#e67e22' },
        watch: { add: '添加察看期', deduct: null, addClass: 'reprimand', deductBg: '#c0392b' }
    };
    const l = labels[S.currentRecordTab] || labels.reward;
    btnAdd.className = 'btn-add-record ' + l.addClass;
    btnAdd.innerHTML = '<i class="fas fa-plus"></i> ' + l.add;
    if (l.deduct) {
        btnDeduct.style.display = 'flex';
        btnDeduct.innerHTML = '<i class="fas fa-minus"></i> ' + l.deduct;
        btnDeduct.style.background = l.deductBg;
    } else {
        btnDeduct.style.display = 'none';
    }
}
...
async function loadRecords() {
    ...
    try {
        ...
        S.currentRecords.mvpRecords = data.mvpRecords || [];
        S.currentRecords.watchRecords = data.watchRecords || [];
        renderRecords();
    } catch (e) {
...
function renderRecords() {
    const listEl = document.getElementById('recordList');
    const recordsMap = {
        reward: S.currentRecords.rewards,
        reprimand: S.currentRecords.reprimands,
        mvp: S.currentRecords.mvpRecords,
        watch: S.currentRecords.watchRecords
    };
    const tabLabels = { reward: '嘉奖', reprimand: '申诫', mvp: 'MVP', watch: '察看期' };
    const records = recordsMap[S.currentRecordTab] || [];

    if (!records || records.length === 0) {
        listEl.innerHTML = `<div class="record-empty">暂无${tabLabels[S.currentRecordTab] || ''}记录</div>`;
        return;
    }
...
async function addRecord(type = 'add') {
    ...
    const endpoints = { reward: 'reward', reprimand: 'reprimand', mvp: 'mvp', watch: 'watch' };
    const endpoint = endpoints[S.currentRecordTab] || 'reward';
...
async function deleteRecord(recordId) {
    ...
    try {
        let url;
        if (S.currentRecordTab === 'mvp') {
            url = `/api/character/${S.currentRecordCharId}/mvp-record/${recordId}`;
        } else if (S.currentRecordTab === 'watch') {
            url = `/api/character/${S.currentRecordCharId}/watch-record/${recordId}`;
        } else {
            url = `/api/character/${S.currentRecordCharId}/record/${recordId}?type=${S.currentRecordTab}`;
        }
...
async function togglePlazaVisibility(charId, hide) {
    try {
        const res = await fetch(`/api/character/${charId}/plaza-visibility`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('ta_token')}` },
            body: JSON.stringify({ visible: !hide })
        });
        const data = await res.json();
        if (data.success) {
            const char = S.allCharacters.find(c => c.id === charId);
            if (char) char.plazaHidden = hide;
            renderCharacters(S.allCharacters);
            showToast(hide ? '已从办公室隐藏' : '已在办公室展示', true);
        }
    } catch (e) { showToast('操作失败'); }
}

async function toggleArchive(charId, archive) {
    try {
        const confirmMsg = archive ? '确定要归档此角色吗？归档后角色主人也无法编辑。' : '确定要取消归档此角色吗？';
        if (!confirm(confirmMsg)) return;
        const res = await fetch(`/api/character/${charId}/archive`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('ta_token')}` },
        ...
        if (data.success) {
            const char = S.allCharacters.find(c => c.id === charId);
            if (char) char.isArchived = archive;
            renderCharacters(S.allCharacters);
            showToast(archive ? '角色已归档' : '已取消归档', true);
        }
    } catch (e) { showToast('操作失败'); }
}
