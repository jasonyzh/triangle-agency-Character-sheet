        ...
        <div class="char-card${c.isArchived ? ' archived' : ''}">
            <div class="card-header" onclick="openSheet('${c.id}')">
                <div class="card-header-actions">
                    <button class="card-top-btn btn-plaza-toggle ${c.plazaHidden ? '' : 'active'}" onclick="event.stopPropagation(); togglePlazaVisibility('${c.id}', ${!c.plazaHidden})" title="${c.plazaHidden ? '在广场中展示' : '在广场中隐藏'}">
...
async function togglePlazaVisibility(charId, hide) {
    try {
        ...
        if (data.success) {
            ...
            renderCharacters(S.allCharacters);
            showToast(hide ? '已从广场隐藏' : '已在广场展示', true);
        }
