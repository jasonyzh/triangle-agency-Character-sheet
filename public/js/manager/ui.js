...
function renderBranchSelector(anchor) {
    ...
    /* desktop.html 嵌入时传入窗口头部的锚点元素；manager.html 无参时找页头按钮组 */
    var header = anchor || document.querySelector('header .btn-group');
    if (!header) return;
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (S.myBranches.length > 0) {
                ...
                if (branchInfo) branchInfo.style.display = 'flex';
            }
        }
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.success && data.branch && data.branch.stats) {
                var scatterEl = document.getElementById('branchScatterDisplay');
                if (scatterEl) scatterEl.textContent = data.branch.stats.total_scatter || 0;
            }
        }
