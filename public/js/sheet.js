    ...
    try {
        ...
        if (!playerSocket) {
            ...
            playerSocket.on('board:image-remove', function(d) {
                if (playerBoard) playerBoard.removeImage(d.imageId);
            });
        }
...
async function switchBoardMission() {
    ...
    try {
        ...
        playerSocket.off('board:image-move').on('board:image-move', function(d) { if (playerBoard) playerBoard.moveImage(d.imageId, d.x, d.y); });
        playerSocket.off('board:image-resize').on('board:image-resize', function(d) { if (playerBoard) playerBoard.resizeImage(d.imageId, d.w, d.h); });
...
function showDiceResult(label, total, detail) {
    ...
    el.innerHTML = '<span class="dice-label">' + label + '</span> <span class="dice-total">' + total + '</span>' + (detail ? ' <span class="dice-detail">' + detail + '</span>' : '');
    ...
    el.style.animation = 'diceFadeIn 0.2s ease';
