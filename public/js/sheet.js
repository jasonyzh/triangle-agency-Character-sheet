...
function updateSwiper() { swiperWrapper.style.transform='translateX(-'+(currentTab*20)+'%)'; document.querySelectorAll('.tab-view')[currentTab].scrollTop=0; updateCharLayout(); requestAnimationFrame(()=>drawTrackSVG()); }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (savedTab && tabOrder.indexOf(savedTab) !== -1) currentTab = tabOrder.indexOf(savedTab);
    ...
    // 恢复 nav 按钮和开关状态
    navBtns.forEach(function(b) { b.classList.remove('active'); });
    if (currentTab === 0) document.querySelector('.nav-btn.n-board').classList.add('active');
    else if (currentTab === 1) document.querySelector('.nav-btn.n-char').classList.add('active');
...
async function loadMailbox() {
    ...
    try {
        ...
        if (u2Svg) {
            u2Svg.style.display = isU2Unlocked ? '' : 'none';
            u2Svg.addEventListener('click', function() { confirmU2Unleash(); });
        }
...
async function deleteMail(msgId) {
    ...
    try {
        await fetch('/api/character/' + charId + '/message/' + msgId, { method: 'DELETE', headers: getAuthHeaders() });
...
function updateSlotButtons() {
    ...
    if (btnAddReal) {
        ...
        if (realCount >= SLOT_LIMITS.realSlots) {
            ...
            if (pc) pc.textContent = '(' + realCount + '/' + SLOT_LIMITS.realSlots + ')';
        } else {
...
function openRealPhone() {
    ...
    pf.addEventListener('animationend', function done() {
        pf.classList.remove('phone-opening');
        pf.style.display = 'block';
        pf.removeEventListener('animationend', done);
    });
}
...
(function() {
    ...
    header.addEventListener('pointerdown', function(e) {
        ...
        sl = pf.offsetLeft; st = pf.offsetTop;
...
function openBriefcase() {
    ...
    bf.addEventListener('animationend', function done() {
        bf.classList.remove('briefcase-opening');
        bf.style.display = 'block';
        bf.removeEventListener('animationend', done);
    });
...
(function() {
    ...
    header.addEventListener('pointerdown', function(e) {
        ...
        dragging = true;
        sx = e.clientX; sy = e.clientY;
        sl = bf.offsetLeft; st = bf.offsetTop;
        header.setPointerCapture(e.pointerId);
    });
    header.addEventListener('pointermove', function(e) {
        if (!dragging) return;
...
function openAnomWindow() {
    if (window.innerWidth < 768) {
        var btn = document.querySelector('.nav-btn.n-anom');
        switchView('view-anom', btn);
        return;
    }
    ...
    wf.addEventListener('animationend', function done() {
        wf.classList.remove('win98-opening');
        wf.style.display = 'block';
        wf.removeEventListener('animationend', done);
    });
}

function closeAnomWindow() {
    var wf = document.getElementById('win98Float');
    var listAnom = document.getElementById('list-anom');
...
(function() {
    ...
    var dragging = false, sx, sy, sl, st;
    if (titleText) {
        titleText.addEventListener('click', function() { addAnom(null, false); });
    }
    ...
    wf.addEventListener('pointerdown', function(e) {
        ...
        dragging = true;
        sx = e.clientX; sy = e.clientY;
        sl = wf.offsetLeft; st = wf.offsetTop;
    });
...
let playerBoardMissions = [];
let currentPlayerBoardMissionId = null;

async function initPlayerBoard() {
    try {
        const res = await fetch(`/api/character/${charId}/mission-boards`, { headers: { 'Authorization': 'Bearer ' + token } });
        playerBoardMissions = await res.json();
        const sel = document.getElementById('boardMissionSelect');
        if (sel) {
            sel.innerHTML = '<option value="">-- 选择任务 --</option>';
            playerBoardMissions.forEach(m => {
                sel.innerHTML += '<option value="' + m.mission_id + '">' + m.mission_name + '</option>';
            });
            // 自动选择上次的任务
            var savedMission = localStorage.getItem('ta_board_mission_' + charId);
            if (savedMission && playerBoardMissions.some(function(m) { return m.mission_id === savedMission; })) {
        ...
        if (!playerSocket) {
            playerSocket = io({ auth: { token: localStorage.getItem('ta_token') } });
            playerSocket.on('connect', () => { console.log('Player board socket connected'); });
            playerSocket.on('mail:new', function() {
                setTimeout(function() { loadMailbox(); }, 500);
            });
            playerSocket.on('board:image-add', function(d) {
                if (playerBoard) playerBoard.addImage({ id: d.id, imageFile: d.imageFile || d.image_lib_filename, x: d.p_x, y: d.p_y, w: d.p_w, h: d.p_h, name: d.name || '', isMapNode: d.is_map_node });
            });
            playerSocket.on('board:image-remove', function(d) {
                if (playerBoard) { playerBoard.removeImage(d.imageId); playerBoard.drawAll(); }
            });
        }
    } catch(e) { console.error('加载任务画板失败', e); }
}

async function switchBoardMission() {
    const sel = document.getElementById('boardMissionSelect');
    const missionId = sel.value;
    const canvasEl = document.getElementById('playerBoardCanvas');
    if (!missionId || !canvasEl) return;
    localStorage.setItem('ta_board_mission_' + charId, missionId);

    if (playerSocket && currentPlayerBoardMissionId) playerSocket.emit('leave-board');
    if (playerBoard) {
        Object.values(playerBoard.images).forEach(el => el.remove());
        if (playerBoard.mapSvg) playerBoard.mapSvg.innerHTML = '';
        if (playerBoard.npcSvg) playerBoard.npcSvg.innerHTML = '';
    }

    try {
        const [boardRes, npcRes] = await Promise.all([
            fetch('/api/board/' + missionId, { headers: { 'Authorization': 'Bearer ' + token } }),
            fetch('/api/board/' + missionId + '/npc-connections', { headers: { 'Authorization': 'Bearer ' + token } })
        ]);
        const data = await boardRes.json();
        const npcConns = await npcRes.json();
        if (!data.success) return;
        currentPlayerBoardMissionId = missionId;

        canvasEl.innerHTML = '';

        playerBoard = new BoardCore(canvasEl, {
            ...
            onImageMove: function(id, x, y) {
                ...
                playerSocket.emit('board:image-move', { imageId: id, x, y }); },
            onImageResize: function(id, w, h) {
                fetch('/api/board/' + missionId + '/image/' + id, {
                    method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
                    body: JSON.stringify({ w, h, role: 'player' })
                });
                playerSocket.emit('board:image-resize', { imageId: id, w, h }); },
            onImageClick: function(imageId) {
                if (npcConnectSourceId && npcConnectSourceId !== imageId) finishNpcConnect(imageId);
            },
            onImageRightClick: function(imageId, x, y) {
                const el = playerBoard.images[imageId];
                if (!el) return;
        ...
        playerBoard.loadImages(data.images);
        if (data.board.show_connections) playerBoard.setConnections(data.connections);
        playerBoard.setNpcConnections(npcConns);

        playerSocket.off('board:image-move').on('board:image-move', function(d) { if (playerBoard) { playerBoard.moveImage(d.imageId, d.x, d.y); playerBoard.drawAll(); } });
        playerSocket.off('board:image-resize').on('board:image-resize', function(d) { if (playerBoard) { playerBoard.resizeImage(d.imageId, d.w, d.h); playerBoard.drawAll(); } });
        playerSocket.off('board:connection-add').on('board:connection-add', function(d) {
            if (playerBoard) {
                if (!playerBoard.connections) playerBoard.connections = [];
                playerBoard.connections.push({ id: d.id, node_a: d.nodeA, node_b: d.nodeB, label: d.label || '' });
                playerBoard.setConnections(playerBoard.connections);
            }
        });
        playerSocket.off('board:connection-remove').on('board:connection-remove', function(d) {
            if (playerBoard) {
                playerBoard.connections = (playerBoard.connections || []).filter(function(c) { return c.id !== d.connId; });
                playerBoard.setConnections(playerBoard.connections);
            }
        });
        playerSocket.off('dice:roll').on('dice:roll', function(d) {
            ...
            showDiceResult('[' + d.charName + '] ' + d.label, d.total, detail);
        });

        if (playerSocket.connected) playerSocket.emit('join-board', { missionId, role: 'ply' });
        else playerSocket.once('connect', () => playerSocket.emit('join-board', { missionId, role: 'ply' }));

        if (window.innerWidth < 768) canvasEl.style.height = Math.max(window.innerHeight * 0.55, 400) + 'px';
        else canvasEl.style.height = '';

        // 重新插入骰子结果容器（已被 innerHTML='' 清除）
...
let playerNpcCtxMenu = null;

function makeCtxMenu(x, y, innerHTML) {
    const div = document.createElement('div');
...
function showPlayerNpcContextMenu(imageId, x, y, missionId) {
    hidePlayerNpcMenu();
    playerNpcCtxMenu = makeCtxMenu(x, y,
    ...
    document.body.appendChild(playerNpcCtxMenu);
    setTimeout(() => { document.addEventListener('mousedown', closeCtx, { once: false }); }, 0);
    function closeCtx(e) { if (!playerNpcCtxMenu || playerNpcCtxMenu.contains(e.target)) return; hidePlayerNpcMenu(); document.removeEventListener('mousedown', closeCtx); }
}

function hidePlayerNpcMenu() { if (playerNpcCtxMenu) { playerNpcCtxMenu.remove(); playerNpcCtxMenu = null; } }

function showNpcLineMenu(connId, x, y, missionId) {
    hidePlayerNpcMenu();
    playerNpcCtxMenu = makeCtxMenu(x, y,
...
async function deleteNpcConnection(connId, missionId) {
    try {
        await fetch('/api/board/' + missionId + '/npc-connection/' + connId, { method: 'DELETE', headers: getAuthHeaders() });
        playerBoard.npcConnections = (playerBoard.npcConnections || []).filter(c => c.id !== connId);
        playerBoard.drawAll();
        showToast('连线已删除', 'success');
    } catch(e) { showToast('删除失败'); }
}

let npcConnectSourceId = null, npcConnectMissionId = null;

function startNpcConnect(imageId, missionId) {
    npcConnectSourceId = imageId;
    npcConnectMissionId = missionId;
    playerBoard.clearHighlight();
    playerBoard.highlightImage(imageId);
    showFloatingCancelBtn();
    showToast('已选起点，点击目标 NPC 完成连线');
}

function showFloatingCancelBtn() {
...
function hideFloatingCancelBtn() {
    var btn = document.getElementById('floatCancelConnect');
    if (btn) btn.remove();
}

function cancelNpcConnect() {
...
function broadcastDiceResult(charName, label, total, results, type) {
    if (currentPlayerBoardMissionId && playerSocket && playerSocket.connected) {
        playerSocket.emit('dice:roll', { missionId: currentPlayerBoardMissionId, charName: charName, label: label, total: total, results: results, type: type });
    }
}

function showDiceResult(label, total, detail) {
...
var diceHistory = JSON.parse(localStorage.getItem('ta_dice_history') || '[]');

function saveToHistory(html) {
    var ts = new Date().toLocaleTimeString();
    diceHistory.unshift({ time: ts, html: html });
    if (diceHistory.length > 50) diceHistory.pop();
    try { localStorage.setItem('ta_dice_history', JSON.stringify(diceHistory)); } catch(e) {}
}
...
function showDiceHistory() {
    ...
    overlay.innerHTML = '<div style="background:#1a252f;border-radius:8px;max-width:400px;width:90%;max-height:80vh;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,0.4);">' +
        ...
        diceHistory.map(function(h) { return '<div style="background:rgba(255,255,255,0.05);color:white;padding:4px 8px;border-radius:4px;font-size:11px;font-family:monospace;"><span style="color:#888;font-size:10px;">' + h.time + '</span> ' + h.html + '</div>'; }).join('') +
        '</div>' +
...
async function finishNpcConnect(targetId) {
    if (!npcConnectSourceId || !npcConnectMissionId || npcConnectSourceId === targetId) return;
    hideFloatingCancelBtn();
    playerBoard.clearHighlight();
    showNpcTypeModal(function(connType) {
        if (!connType) { npcConnectSourceId = null; return; }
        createNpcConnection(npcConnectSourceId, targetId, connType);
    });
}
...
async function createNpcConnection(nodeA, nodeB, connType) {
    try {
        const res = await fetch('/api/board/' + npcConnectMissionId + '/npc-connection', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
            body: JSON.stringify({ nodeA, nodeB, connType })
        });
        const d = await res.json();
        if (d.success) {
            if (!playerBoard.npcConnections) playerBoard.npcConnections = [];
            playerBoard.npcConnections.push({ id: d.id, node_a: d.nodeA, node_b: d.nodeB, conn_type: connType, label: '' });
            playerBoard.drawAll();
            showToast('连线已创建', 'success');
        }
        npcConnectSourceId = null;
    } catch(e) { showToast('连线失败'); npcConnectSourceId = null; }
}

async function promptNpcLabel(connId, missionId) {
    const conn = (playerBoard.npcConnections || []).find(c => c.id === connId);
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:20000;display:flex;align-items:center;justify-content:center;';
...
async function showImagePopup(imageId) {
    ...
    // Resize handler - proportional bottom-right corner drag
    ...
    // Drag handler - constrain to viewport
