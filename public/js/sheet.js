...
const tabOrder = ['view-char', 'view-anom', 'view-real', 'view-item', 'view-board'];
...
function updateSwiper() { swiperWrapper.style.transform='translateX(-'+(currentTab*20)+'%)'; navBtns.forEach((b,i)=>{if(i===currentTab)b.classList.add('active');else b.classList.remove('active');}); document.querySelectorAll('.tab-view')[currentTab].scrollTop=0; updateCharLayout(); requestAnimationFrame(()=>drawTrackSVG()); }
...
async function switchBoardMission() {
    ...
    try {
        ...
        canvasEl.style.height = Math.max(window.innerHeight * 0.55, 400) + 'px';
