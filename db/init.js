...
// 分部图标与介绍短语（分部管理）
['icon', 'intro'].forEach(col => {
    const has = db.pragma('table_info(branches)').some(c => c.name === col);
    if (!has) {
        console.log(`正在添加 ${col} 字段到 branches 表...`);
        try {
            db.exec(`ALTER TABLE branches ADD COLUMN ${col} TEXT DEFAULT ''`);
...
if (!fieldMissionsColNames.includes('weather')) {
    db.exec('ALTER TABLE field_missions ADD COLUMN weather TEXT DEFAULT ""');
}
...
db.exec(`CREATE TABLE IF NOT EXISTS anomaly_templates (
    ...
    p2 TEXT DEFAULT '[]',
    p3 TEXT DEFAULT '[]',
    sub_on INTEGER DEFAULT 0,
    sub TEXT DEFAULT '',
    list_on INTEGER DEFAULT 0,
    alist_name TEXT DEFAULT '',
    alist TEXT DEFAULT '[]',
...
// 旧库迁移：anomaly_templates 加 passive / t3 / t3v / p3 列
try {
    const anomTplCols = db.pragma('table_info(anomaly_templates)').map(c => c.name);
    if (!anomTplCols.includes('passive')) {
        db.exec('ALTER TABLE anomaly_templates ADD COLUMN passive INTEGER DEFAULT 0');
    }
    if (!anomTplCols.includes('t3')) {
        db.exec('ALTER TABLE anomaly_templates ADD COLUMN t3 TEXT DEFAULT ""');
    }
    if (!anomTplCols.includes('t3v')) {
        db.exec('ALTER TABLE anomaly_templates ADD COLUMN t3v TEXT DEFAULT ""');
    }
    if (!anomTplCols.includes('p3')) {
        db.exec("ALTER TABLE anomaly_templates ADD COLUMN p3 TEXT DEFAULT '[]'");
    }
    if (!anomTplCols.includes('sub_on')) {
        db.exec('ALTER TABLE anomaly_templates ADD COLUMN sub_on INTEGER DEFAULT 0');
    }
    if (!anomTplCols.includes('sub')) {
        db.exec('ALTER TABLE anomaly_templates ADD COLUMN sub TEXT DEFAULT ""');
    }
    if (!anomTplCols.includes('list_on')) {
        db.exec('ALTER TABLE anomaly_templates ADD COLUMN list_on INTEGER DEFAULT 0');
    }
    if (!anomTplCols.includes('alist')) {
        db.exec("ALTER TABLE anomaly_templates ADD COLUMN alist TEXT DEFAULT '[]'");
    }
    if (!anomTplCols.includes('alist_name')) {
        db.exec("ALTER TABLE anomaly_templates ADD COLUMN alist_name TEXT DEFAULT ''");
    }
} catch (e) {}


db.exec(`CREATE TABLE IF NOT EXISTS npc_templates (
    id TEXT PRIMARY KEY,
    branch_id TEXT NOT NULL,
    name TEXT NOT NULL,
    actor TEXT DEFAULT '',
