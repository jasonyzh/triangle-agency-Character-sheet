...
function checkManagerAuthorization(managerId, targetUserId) {
    const sql = `
        ...
        LIMIT 1
    `;
    const row = db.prepare(sql).get(managerId, targetUserId);
    return !!row;
}
...
function checkManagerCharacterAuth(managerId, charId) {
    ...
    const row = db.prepare('SELECT 1 FROM user_branches WHERE user_id = ? AND branch_id = ?').get(managerId, charRow.branch_id);
    return !!row;
}
